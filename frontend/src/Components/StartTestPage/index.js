...
const StartTestPage = () => {
    ...
    useEffect(() => {
        ...
        async function fetchTest() {
            try {
                ...
                const response = await fetch('http://localhost:8081/users/current', { //получить пользователя
                    credentials: "include",
                });
                if (!response.ok) {
                ...
                const response1 = await fetch('http://localhost:8081/test/id:' + testid);
                if (!response1.ok) {
                    throw new Error('Ошибка сети');
                }
                ...
                const response2 = await fetch('http://localhost:8081/test/find-testing-attempt-by-test', {
                    method: 'POST',
                    headers: {
                ...
                const response3 = await fetch('http://localhost:8081/test/get-tasks-test', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8'
                    },
    ...
    async function StartTrainingTest() {
        ...
        const response = await fetch('http://localhost:8081/test/create-test-simulator', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json;charset=UTF-8'
            },
