...
const CreateSubjectPage = ({editItem, onCreate, onError}) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if(editItem==null){
                const response = await fetch('http://localhost:8081/subject/add', { // добавить предмет
                    method: 'POST',
                    headers: {
                ...
                const response = await fetch('http://localhost:8081/subject/update-subject', { // добавить предмет
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8'
                    },
