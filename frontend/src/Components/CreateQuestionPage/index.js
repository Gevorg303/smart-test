    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if(editItem==null) {
                ...
                const response = await fetch('http://localhost:8081/task/update-task', {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json;charset=UTF-8'
                    },
    ...
    useEffect(() => {
        async function fetchSubjects() {
            try {
                ...
                const response3 = await fetch('http://localhost:8081/TypeTask/all');
                if (!response3.ok) {
        ...
        async function setData() {
            if (editItem != null) { //выполняется если предается предмет который нужно изменить
                ...
                try {
                    const response2 = await fetch('http://localhost:8081/task-of-indicator/find-indicator-by-task', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json;charset=UTF-8'
                        },
                ...
                try {
                    const response2 = await fetch('http://localhost:8081/response-option/find-response-option-by-task', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json;charset=UTF-8'
                        },
